...
export class TypeAnalyzer {
  ...
  shouldIgnoreType(typeName, filePath = '') {
    ...
    if (this.options.ignoreVueComponentTypes && filePath.endsWith('.vue')) {
      const vueCommonTypes = ['Props', 'Emits', 'Slots', 'Expose', 'Data', 'Methods', 'Computed'];
      ...
      if (importDecl.importClause?.namedBindings && ts.isNamedImports(importDecl.importClause.namedBindings)) {
        ...
        for (const element of namedImports.elements) {
          ...
          if (!usedIdentifiers.has(importName)) {
            const position = ts.getLineAndCharacterOfPosition(sourceFile, element.pos);
            this.warnings.push({
              type: "unused-import",
              file: sourceFile.fileName,
              line: position.line + 1,
    ...
    for (const [fileName, imports] of this.importMap) {
      ...
      for (const importInfo of imports) {
        if (importInfo.imports.includes(typeName)) {
          ...
          if (importInfo.module.includes(relativePath) || importInfo.module.includes(typeName)) {
            return true;
          }
        }
      }
  ...
  generateReport() {
    ...
    const totalUsages = Array.from(this.usageMap.values()).reduce(
      (sum, usages) => sum + usages.length, 0
    );
  ...
  calculateHealthScore() {
    ...
    return Math.max(0, Math.round(100 - unusedPenalty - duplicatePenalty + complexityBonus));
  }
  ...
  calculateValidationScore() {
    ...
    return Math.max(0, Math.round(100 - criticalPenalty - errorPenalty - warningPenalty));
  }
  ...
  generateSuggestions() {
    ...
    if (this.unusedTypes.size > 0) {
      suggestions.push(`💡 发现 ${this.unusedTypes.size} 个未使用的类型定义，建议清理`);
    }

    if (this.duplicates.size > 0) {
      suggestions.push(`⚠️ 发现 ${this.duplicates.size} 个重复的类型定义，建议合并`);
    }

    const unusedImports = this.warnings.filter(w => w.type === "unused-import").length;
    if (unusedImports > 0) {
...
// 文件扫描工具函数
function scanFiles(startDir, extensions = ['.ts', '.tsx'], exclude = []) {
  const files = [];
    ...
    try {
      ...
      for (const item of items) {
        ...
        if (item.isDirectory()) {
          ...
          const ext = require('path').extname(item.name);
