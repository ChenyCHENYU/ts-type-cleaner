...
export class TypeAnalyzer {
  ...
  async initializeTypeScriptProgram() {
    ...
    try {
      ...
      if (this.options.verbose) {
        console.log(
          `📁 TypeScript 程序初始化完成，包含 ${fileNames.length} 个文件`
        );
      }
    } catch (error) {
  ...
  async analyzeWithAST() {
    ...
    for (const sourceFile of sourceFiles) {
      ...
      const isProjectFile = this.sourceFiles.some(
        (file) =>
      ...
      try {
        ...
        console.warn(
          `⚠️ 分析文件失败 ${sourceFile.fileName}: ${error.message}`
        );
      }
    }
    ...
    for (const filePath of vueFiles) {
      try {
        ...
        const scriptMatch =
          ...
          content.match(
            /<script[^>]*setup[^>]*lang=["']ts["'][^>]*>([\s\S]*?)<\/script>/i
          ) ||
        ...
        if (scriptMatch) {
          ...
          try {
            ...
            console.warn(
              `⚠️ Vue 文件 TypeScript 解析失败 ${filePath}: ${error.message}`
            );
          }
        }
      } catch (error) {
  ...
  async runSemanticAnalysis() {
    ...
    try {
      ...
      if (this.options.verbose) {
        console.log(
          `🔍 语义分析完成，发现 ${this.errors.length} 个错误，${this.warnings.length} 个警告`
        );
      }
    } catch (error) {
  ...
  checkAnyTypeUsage(sourceFile) {
    ...
    try {
      ...
      console.warn(
        `⚠️ any 类型检查失败 ${sourceFile.fileName}: ${error.message}`
      );
    }
  }
  ...
  checkUnusedImports(sourceFile) {
    try {
      ...
      console.warn(
        `⚠️ 未使用导入检查失败 ${sourceFile.fileName}: ${error.message}`
      );
    }
  }
  ...
  detectUnusedTypes() {
    try {
      ...
      if (this.options.verbose) {
        console.log(
          `🔍 未使用类型检测完成，发现 ${unusedTypes.size} 个未使用类型`
        );
      }
    } catch (error) {
  ...
  calculateHealthScore() {
    try {
      ...
      const unusedPenalty = Math.min(40, unusedRatio * 60);
      score -= unusedPenalty;

      // 重复定义扣分 (最多扣30分)
      const duplicateRatio = this.duplicates.size / totalTypes;
      const duplicatePenalty = Math.min(30, duplicateRatio * 50);
      score -= duplicatePenalty;

      // 类型复杂度奖励 (最多加5分，但总分不超过100)
      const complexityBonus = Math.min(5, Math.log10(totalTypes + 1) * 2);
      score = Math.min(100, score + complexityBonus);
  ...
  calculateValidationScore() {
    try {
      ...
      score -= criticalErrors * 15; // 每个关键错误扣15分

      // 普通错误扣分
      const regularErrors = this.errors.length - criticalErrors;
      score -= regularErrors * 8; // 每个普通错误扣8分

      // 警告轻微扣分
      score -= Math.min(20, this.warnings.length * 1); // 每个警告扣1分，最多扣20分
