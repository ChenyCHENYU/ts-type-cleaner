...
export class TypeAnalyzer {
  ...
  findUnusedTypes() {
    ...
    for (const [typeName, definition] of this.types.definitions) {
      ...
      if (this.verbose) {
        console.log(`检查类型 ${typeName}:`, {
          定义位置: `${this.relativePath(definition.file)}:${definition.line}`,
          使用次数: usages.length,
          使用位置: usages.map(u => `${this.relativePath(u.file)}:${u.line}`)
        })
      }
      ...
      const actualUsages = usages.filter(usage => {
        ...
        return lineDiff > 5 // 放宽到5行以上才算真实使用
      })
