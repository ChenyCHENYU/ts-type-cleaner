...
export class TypeAnalyzer {
  constructor(options = {}) {
    ...
    this.ignorePatterns = options.ignorePatterns || [/^Props$/, /^Emits$/, /^Slots$/, /^Expose$/]
  ...
  isProjectFile(fileName) {
    ...
    return !normalized.includes('node_modules') && 
           ...
           this.sourceFiles.some(f => resolve(f).replace(/\\/g, '/') === normalized)
  }
  ...
  extractTypesFromVueFile(filePath) {
    try {
      ...
      const scriptMatch = content.match(/<script[^>]*(?:\s+lang=["'](?:ts|typescript)["']|\s+setup)[^>]*>([\s\S]*?)<\/script>/i)
  ...
  shouldIgnoreType(typeName) {
    const builtins = ['string', 'number', 'boolean', 'Array', 'Promise', 'Record', 'Partial']
    ...
    for (const diagnostic of diagnostics) {
      ...
      if (this.isImportantError(diagnostic)) {
        const issue = {
          ...
          line: diagnostic.start ? ts.getLineAndCharacterOfPosition(diagnostic.file, diagnostic.start).line + 1 : 0,
  ...
  isImportantError(diagnostic) {
    ...
    if (message.includes('import.meta') || message.includes('vite')) return false
  ...
  generateReport() {
    ...
    const stats = {
      ...
      usageReferences: Array.from(this.typeUsages.values()).reduce((sum, uses) => sum + uses.length, 0),
    ...
    const validationScore = this.errors.length === 0 ? 100 : Math.max(0, 100 - this.errors.length * 10)
    ...
    return {
      ...
      details: {
        ...
        errors: this.errors.map(e => ({ ...e, file: this.getRelativePath(e.file) })),
        warnings: this.warnings.map(w => ({ ...w, file: this.getRelativePath(w.file) }))
      },
  ...
  getUnusedTypes() {
    ...
    for (const [typeName, definition] of this.typeDefinitions) {
      ...
      const realUsages = usages.filter(usage => 
        !(usage.file === definition.file && Math.abs(usage.line - definition.line) <= 2)
      )
  ...
  calculateHealthScore(stats) {
    ...
    score -= Math.min(30, (stats.duplicateDefinitions / stats.typeDefinitions) * 40)
