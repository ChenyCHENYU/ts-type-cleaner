    ...
    try {
      ...
      if (this.typeDefinitions.has(name)) {
        ...
        if (existing.file !== info.file) {
          ...
          // 检查是否已经添加过这个定义
          const existingDuplicates = this.duplicateTypes.get(name);
          const alreadyExists = existingDuplicates.some(
            (dup) => dup.file === info.file && dup.line === info.line
          );
          ...
          if (!alreadyExists) {
            ...
            if (this.options.verbose) {
              ...
              console.log(
                `    - ${relative(this.options.rootDir, existing.file)}:${
                  existing.line
                }`
              );
              console.log(
                `    - ${relative(this.options.rootDir, info.file)}:${
                  info.line
                }`
              );
            }
