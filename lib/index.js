      ...
      if (this.typeDefinitions.has(name)) {
        ...
        if (existing.file !== info.file) {
          ...
          if (this.options.verbose) {
            ...
            console.log(`    - ${relative(this.options.rootDir, existing.file)}:${existing.line}`)
            console.log(`    - ${relative(this.options.rootDir, info.file)}:${info.line}`)
          }
        }
        // 同文件中的定义，更新为最新的
        else {
  ...
  async detectUnusedTypes() {
    try {
      for (const [typeName, typeInfo] of this.typeDefinitions) {
        ...
        const isCommonType = commonTypePatterns.some(pattern => pattern.test(typeName))
        if (isCommonType) {
          continue
        }
      ...
      if (this.options.verbose) {
        ...
        if (this.unusedTypes.size > 0) {
          console.log('前几个未使用类型:', Array.from(this.unusedTypes).slice(0, 5))
        }
      }
    } catch (error) {
      if (this.options.verbose) {
